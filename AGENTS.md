...
- 모든 P3 finding은 finding ID, exact base/head, 처분, 근거, 검증, 책임자를 PR 기록에 남기고 thread를 해소한다. P3 처분은 수정/기각/위험수용/후속 이슈 중 하나여야 한다. 기각에는 반증을, 위험수용에는 만료일을, 후속 이슈에는 URL·owner·milestone·완료 조건을 남긴다. reviewer 간 severity 불일치는 상위 등급을 적용하며 작성자 단독으로 하향할 수 없다.
...
- 최종 이중 리뷰가 같은 base/head에서 `P0=0, P1=0, P2=0`, P3 처분 완료, unresolved 0을 만족한 뒤에만 PR에 `reviews-complete` label을 새로 붙여 `pull-request-validation`을 실행한다. workflow는 label 추가 이벤트에서 현재 PR merge candidate를 테스트하고 base/head/merge-candidate SHA를 기록한다. label은 리뷰 완료를 증명하지 않고 검증 시작 순서 제어 신호일 뿐이다.
