...
- 완성된 head에서 정확한 base ref, 최신 base SHA와 head SHA를 기록하고 `repo:base_sha:head_sha:round` 형식의 review key를 만든다.
- Cursor/local 작업 시 final review 순서는 **독립 reviewer 먼저 → 통과 후에만 `@codex review`** 이다. 같은 exact base/head에서 native 독립 review를 1회 먼저 돌리고, 그 결과가 `P0=0, P1=0, P2=0`일 때만 GitHub `@codex review`를 1회 호출한다. 독립 리뷰에 P0/P1/P2가 있으면 `@codex review`를 호출하지 않고 RCA 게이트로 수정한 뒤 새 head에서 독립 리뷰부터 다시 한다.
- 두 리뷰의 finding과 결론은 서로 전달하지 않는다. 각 리뷰는 호출된 시점의 exact head를 대상으로 하며, 해당 리뷰가 terminal이 될 때까지 그 head를 바꾸지 않는다.
...
- 유효한 P0/P1/P2 finding이 하나라도 나오면, 이미 호출된 final review가 모두 terminal이 된 뒤에만 수정을 시작한다. 독립 리뷰만 돌린 상태에서 P0/P1/P2가 나오면 `@codex review` 없이 바로 RCA 수정으로 들어간다. 수정 중·수정 직후에는 `@codex review`와 독립 reviewer subagent를 호출하지 않는다. finding을 “지적된 줄 패치 → 즉시 재리뷰”로 소비하는 것을 금지한다.
...
- 지적된 줄만 고치거나 reviewer 문구를 옮겨 즉시 재검토하지 않는다. main Codex가 새 head를 완성됐다고 판정한 뒤에만 새 exact base/head와 새 round의 review key에서 독립 review를 다시 1회 수행하고, 통과한 뒤에만 `@codex review`를 1회 수행한다.
...
- 리뷰 뒤 push로 head SHA가 바뀌거나 base SHA가 이동하면 이전 GitHub Codex review와 독립 review를 모두 stale로 처리한다. 새 head를 다시 완성한 뒤 새 exact base/head와 새 review key에서 독립 review를 먼저 다시 하고, 통과한 뒤에만 `@codex review`를 반복한다. 최종 두 리뷰가 같은 최신 base/head 쌍을 검토하지 않았다면 merge하지 않는다.
