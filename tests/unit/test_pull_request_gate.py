...
class PullRequestGateUnitTest(unittest.TestCase):
    ...
    def test_checked_in_workflows_and_ruleset_keep_fixed_gate_contract(self) -> None:
        ...
        self.assertIn("ConvertTo-CanonicalRulesetValue", configure_script)
        self.assertIn("Sort-Object -CaseSensitive", configure_script)
        self.assertIn("Write-Output -NoEnumerate", configure_script)
        self.assertIn("did not converge after 5 attempts", configure_script)
        self.assertIn("Start-Sleep -Seconds 1", configure_script)
        self.assertIn("Get-EffectiveRuleProbeBranchNames", configure_script)
        self.assertIn("ToUnixTimeMilliseconds", configure_script)
        self.assertIn("$rawRules | ForEach-Object", configure_script)
        self.assertIn('Where-Object { $_.Name -eq "parameters" }', configure_script)
