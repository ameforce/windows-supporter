...
- 정리: 보호된 remote hotfix/release branch 삭제 전 exact ref에 creation/update freeze를 적용한다. canonical ruleset에서는 exact ref만 일시 exclude하고 leased compare-and-delete를 수행한다. `finally`에서 canonical 보호를 먼저 복원한 뒤 freeze 상태에서 remote ref 부재를 재확인하고, freeze 제거 후에도 canonical 일치와 ref 부재를 최종 확인한다.
